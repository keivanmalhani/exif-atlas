...
https://github.com/keivanmalhani/exif-atlas and choose **Report a
...
- a crafted EXIF field in a photograph causing command execution, an unexpected
  file write, an outbound network request, or terminal escape sequences
...
- writing outside the output path you asked for, or overwriting something you
  did not name
- a crafted EXIF field reaching the generated HTML unescaped, since the report
  is a page somebody will open in a browser
- a coordinate reaching the report when the run asked for it to be omitted,
  which is a privacy failure rather than a crash
...
reads the metadata headers of the files under a folder you name, never decoding
an image, and writes one self contained HTML report where you tell it to. It
makes no network request, and the report it writes fetches nothing when opened.
